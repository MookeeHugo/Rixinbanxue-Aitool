...
| 英文/缩写 | 中文含义 | 在文档中的使用场景 |
| --- | --- | --- |
| E2E (End-to-End) | 端到端自动化测试 | 强调通过 Playwright/Cypress 模拟教师完整流程，确保组卷/批改闭环。 |
| Pre-commit hook | Git 提交前钩子 | 与本报告建议的 UTF-8 巡检、编码守护流程联动，在提交前阻断乱码。 |
| Hydration | SSR 再水合过程 | 解释退出登录后 `_next` 静态资源 404 导致 React 无法 hydrate 的根因。 |
| Middleware | Next.js 中间件守卫 | 描述在 `middleware.ts` 中放行 `/_next` 等静态资源的修复动作。 |
| Webhook nonce | Webhook 随机校验串 | 结合 LiveKit/ZEGO Webhook 提醒需要写入 Redis/缓存以阻止重放攻击。 |
| RLS (Row Level Security) | 行级安全策略 | Supabase 表策略缺少 `WITH CHECK` 的问题，需要约束 `teacher_id`/`created_by`。 |
| HMR (Hot Module Replacement) | 热模块替换/热更新 | 开发模式 500 章节中提及 `webpack-hmr`，用于说明 dev server 失败的连锁反应。 |
